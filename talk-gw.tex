...
\title{Nonparametric Bayesian System Reliability\\ with Sets of Priors}
...
\alert{Sets of priors}}
 \begin{itemize}
 \item model uncertainty in probability statements ***show lottery A, B box
 \item allow for partial or vague information on $\ptk$
 \item highlight prior-data conflict.
 \end{itemize}
...
vary $(\nzg, \yzr)$ in a set $\PZc = [\nzlg, \nzug] \times [\yzlr, \yzur]$\\
\uncover<1->{\quad\play\ easy elicitation, tractability \& prior-data conflict sensitivity}\\
Bounds for inferences (point estimate, \ldots) by min/max over $\PZc or \PNc$
